...
- 联网搜索方式改为**用户手动选择**（DuckDuckGo / Tavily / DeepSeek 联网搜索），可自由切换，配置不满足时给出明确提示
- AI 任务串行执行（一次一个），避免并发调用不同搜索 C 库导致的进程崩溃
- AI 补全 / 总结完成后自动刷新卡片草稿（无未保存修改时同步显示结果；用户编辑中则不覆盖）
- AI 补全增强：自动检索前三位作者的 Google Scholar 学术信息（引用量 / 研究方向 / 单位），按「人名：介绍」填入作者团队信息与主要作者研究方向；创新点按 `1.` `2.` 编号格式书写
- 修复：arXiv 查询网络异常/超时不再导致补全任务整体失败，而是降级继续（跳过 arXiv，走网页搜索 + LLM）
